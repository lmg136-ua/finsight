...
def verify_claims(state: GraphState) -> GraphState:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an Evidence Verifier in a financial research system.\n"
                   ...
                   "For each important claim, determine if it is SUPPORTED, PARTIALLY_SUPPORTED, or UNSUPPORTED.\n"
                   "If any important claim is UNSUPPORTED or PARTIALLY_SUPPORTED, set `all_supported` to False, and provide feedback on what needs to be fixed.\n"
                   "If the draft states that information is missing, that is a valid response and should not be penalized, as long as it doesn't hallucinate fake numbers.\n"
                   "Context:\n{context}"),
        ("user", "Draft Answer:\n{draft}")
    ])
